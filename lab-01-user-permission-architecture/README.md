...
- The auditor cannot modify application resources.
...
2. Switch to user `dev1` and verify write access to `/srv/app`.
3. Switch to user `auditor` and attempt to modify `/srv/app` (should fail).
4. Confirm permission enforcement using `ls -l`.
